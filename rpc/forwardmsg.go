...
/*ForwardMsgWrap 将消息封装为转发消息
  target 转发目标
  msg 被编码的消息*/
func ForwardMsgWrap(target int32, msg proto.Message) (*ForwardMsg, error) {
	buf, err := proto.Marshal(msg)
	h := fnv.New32a()
	h.Write([]byte(reflect.TypeOf(msg.(proto.Message)).Elem().Name()))
	return &ForwardMsg{Target: target, Msg: &RawMsg{Type: int32(h.Sum32()), Raw: buf}}, err
}

/*ForwardMsgHandle 转发消息处理函数
  fmsg 转发消息
  stream 消息流*/
func ForwardMsgHandle(fmsg *ForwardMsg, stream interface{}) error {
